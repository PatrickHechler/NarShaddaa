...
public class AssymetrischPaar implements AssymetrischPaarInterface {
	...
	public AssymetrischPaar(int schlüsselstürke) {
		KeyPair paar = generiereNeuesPaar(schlüsselstürke);
		generiereVomPaar(paar);
	}
	...
	private KeyPair generiereNeuesPaar(int schlüsselstürke) {
		KeyPairGenerator rsaKPG;
		try {
			rsaKPG = KeyPairGenerator.getInstance("RSA");
			rsaKPG.initialize(schlüsselstürke);
	...
	public AssymetrischPaar(PrivateKey privaterSchlüssel, PublicKey üffentlicherSchlüssel) {
		eigener = new AssymetrischEigener(privaterSchlüssel);
		offen = new AssymetrischOffen(üffentlicherSchlüssel);
	}
